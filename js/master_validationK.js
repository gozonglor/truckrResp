...
//example url: ...clients.html?user=glor&ABC123
	...
	if (typeof query !== "undefined"){
		...
		currentUser = name;
		ekey = query[1];
		document.getElementById("hidden_username").innerHTML = currentUser;
		document.getElementById('show_user_status').innerHTML = currentUser;
				...
				var response =$.ajax({
					...
					success: function (response) {
						...
						if (response[0] == "False"){ //if there is no org tied to the donor
							formOwnership = chosenClientFName.toUpperCase()+" "+chosenClientLName.toUpperCase()+" ";
						}
					
						formHtml = "";
						document.getElementById("customTOOForm").innerHTML = formOwnership+response[1];
						var data = formOwnership+response[1];
						formHtml = data;
		...
		var valid = validateSignature();
			if (valid === true){	
				...
				if (sig_coord !== ""){
					...
					var api = $('.signed').signaturePad({displayOnly:true}); //--yposs
...
function checkTOOForm(){
	...
	if (((document.getElementById("client_title").value === "")) || (document.getElementById("p4p_name").value === "")){
		...
		if (customer_signature === false){ document.getElementById("error_message_transfer").innerHTML = "Missing fields."; }	
		else{validate = true;}
	}
	return validate;
}
...
//[ testing code ] **? dead
//clicking the show button to reveal the agreement form. 
//used to simulate the hiding/showing between each module
$(document).ready(function(){
	$("#show_button").click(function(){
		$("#too_agreement_form").show();
	});
});


//[ testing code ] **? dead
//current not called anywhere else in the code
function checkIfTOO(){//check if transfer of ownership form div/module is displayed
	var tooDiv = document.getElementById("too_agreement_form"); //grab the div containing the transfer of ownership form
	if (tooDiv.is(':visible')){
		var lotNum = generateLotNum(); //if we have shown the div, then generate the lot number
		lotNumField.value = lotNum;
	}
}
...
//**? dead, not called anywhere else
function buildQuery(){
//http://www.designchemical.com/blog/index.php/jquery/8-useful-jquery-snippets-for-urls-querystrings/
	var url = document.URL;//get the current page url
	var root = location.protocol + '//' + location.host;//get current root url
...
function checkFinalForm(){
	var valid = true;
	if ((document.getElementById("client_title").value == "") || (document.getElementById("p4p_name").value == "")){valid = false;}
	return valid;
}

//**? dead, not called anywhere else
function emailTo(clientEmail){
	var email = clientEmail;
	var subject = "PCs for People: Transfer of Ownership Agreement";
	var body = "Hello";
	var link = "mailto:"+clientEmail
			 + "?cc=glor@pcsforpeople.com"
...
function demoFromHTMLOLD(){
		...
		var p4pLogo = 'data:image/jpeg;base64,';
		
		var canvas = document.getElementById("canvas_pad");
		
		
		var donorSig = canvas.toDataURL("image/jpeg");
		
		//doc.text("HELLO");
		doc.addImage(donorSig,'JPEG',2,10);
		doc.addImage(p4pLogo,'JPEG', 0.5,0.5);
		var holder = doc.output('dataurlstring');
		 
		document.getElementById('test_2').innerHTML = "Holder$"+holder.trim();
		
		 //doc.output('dataurlstring');
		 		 //alert("important : "+doc.output('dataurlstring');

		 //document.getElementById("test_2").innerText = doc.output('dataurlstring');
		 //alert("important : "+doc.output('dataurlstring');
}
			...
			var response =$.ajax({
				...
				success: function (response) {
					...
					if (response == true){		
						...
						document.getElementById("message_transfer").innerHTML = "Transfer Form successfully added to database. Email sent.";
